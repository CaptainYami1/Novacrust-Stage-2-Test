...
export const MainForm = () => {
  ...
  const rate = selectedToken ? TOKEN_RATES[selectedToken.name] : undefined;
  const receiveAmount =
    payAmount && Number(payAmount) > 0 && rate
      ? (Number(payAmount) * rate).toFixed(2)
      : "";
        ...
        <div className="flex flex-col gap-2 p-6 border border-[#E0E0E0] rounded-[30px] ">
          ...
          <div className="flex items-center justify-between">
            <input
              ...
              placeholder="1.00"
              value={payAmount}
              className="w-full min-w-0 truncate focus:outline-none no-spinners text-2xl text-[#000E10] font-semibold outfit-font placeholder:text-[#000E10]"
        ...
        <div className="flex flex-col gap-2 p-6 border border-[#E0E0E0] rounded-[30px] ">
          ...
          <div className="flex items-center justify-between">
            <input
              ...
              value={receiveAmount}
              readOnly
              className=" w-full min-w-0 truncate focus:outline-none flex-wrap no-spinners text-2xl text-[#000E10] font-semibold outfit-font placeholder:text-[#000E10]"
            />
            <CurrencySelector />
          </div>
        </div>
        <InputSelectorDropdown
          InputOption={WalletOptions}
          title="Pay from"
          selectedInput={payFrom}
          onSelect={setPayFrom}
        />
        <InputSelectorDropdown
          InputOption={ReceiveOptions}
          title="Pay to"
          selectedInput={receiveTO}
          onSelect={setReceiveTO}
        />
      </div>
